...
DEPTH_MARKER_RE = re.compile(
    r"^<!--\s*(?:DESCENT:L\d+:M\d+(?:\s*,\s*M\d+)*|SPARK:S\d+|REBOUND)\s*-->$",
    flags=re.IGNORECASE,
)
...
def article_mode(topic_dir: Path) -> str:
    ...
    return (
        mode
        if mode in {"technical_explainer", "hands_on_playbook"}
...
class Theme:
    ...
    def __init__(
        ...
        *,
        ...
        self.mode = mode
        self.is_life = mode == "life_insight"
...
def render_code_block(lines: list[str], theme: Theme) -> str:
    ...
    for line_number, line in enumerate(code_lines, start=1):
        ...
        rendered_lines.append(
            ...
            f'font-size: 12px; line-height: 1.6; color: {theme.code_ink}; text-align: left; '
    ...
    return (
        ...
        f'font-size: 12px; line-height: 1.4; color: {theme.code_muted}; letter-spacing: 1px; '
        ...
        f'font-size: 12px; letter-spacing: 2px;"><span leaf="">•••</span></span></section>\n'
