"""Path rules for wechat-article outputs."""
...
def latest_workflow_path() -> Path:
    return wechat_workspace_root() / "_runtime" / "wechat_workflow" / "latest.json"
...
def ensure_topic_dir(path: Path) -> Path:
    ...
    try:
        ...
        fail(
            ...
            f"All wechat-article outputs must live under {root}."
        )
    return topic_dir
