...
def convert_inline(text: str, theme: "Theme") -> str:
    ...
    escaped = re.sub(
        r"\*\*(.*?)\*\*",
        rf'<strong data-wa-format="inline-highlight" style="color: {theme.emphasis}; font-weight: 900; '
        rf'border-bottom: 2px solid {theme.subsection}; padding: 0 1px 2px;">\1</strong>',
        escaped,
    )
    escaped = re.sub(
        r"\*(.*?)\*",
...
def split_sentence_lines(text: str) -> list[str]:
    """Split prose into complete semantic sentences without breaking inline markup."""
    text = text.strip()
    if not text:
        return []
    ...
    while index < len(text):
        if text[index] == "`":
            ...
            index += 1
            continue
        if not code_open and text.startswith("**", index):
        ...
        if (
            not code_open
            and not strong_open
            and text[index] in SENTENCE_ENDINGS
        ):
            end = index + 1
    ...
    if tail:
        if parts and nonspace_len(tail) <= 3:
...
def expand_sentence_line_blocks(
    blocks: list[tuple[str, list[str]]],
) -> list[tuple[str, list[str]]]:
    """Make one complete prose sentence one HTML paragraph.

    Markdown remains content-first. The renderer owns this presentational
    rhythm, so a well-written sentence line survives even when the model used
    soft line breaks instead of blank lines.
    """
    expanded: list[tuple[str, list[str]]] = []
    for block_type, lines in blocks:
        if block_type != "paragraph" or image_id_from_block(block_type, lines):
    ...
    def __init__(
        ...
        *,
        ...
        if self.is_life:
            ...
            # system: rain blue, warm copper, dark wine and grey green.
            self.section = "#526A7A"
            self.subsection = "#B67850"
            self.emphasis = "#874F52"
            self.tertiary = "#718274"
        else:
...
def render_article_title(text: str, theme: Theme) -> str:
    ...
    return (
        f'<h1 style="font-size: 15px; line-height: 1.55; color: {theme.ink}; font-weight: 900; '
        f'margin: 12px 0 26px; padding: 15px 10px 16px; text-align: center; letter-spacing: 0; '
        f'border-top: 3px solid {theme.section}; border-bottom: 1px solid {theme.hairline}; '
        f'box-sizing: border-box;">{convert_inline(text, theme)}</h1>'
    )


def render_quote(text: str, theme: Theme, accent: str | None = None) -> str:
    ...
    if theme.is_life:
        return (
            ...
            f'border-bottom: 1px solid {theme.hairline}; border-left: 3px solid {accent}; '
    ...
    return (
        f'<section data-wa-format="quote" style="margin: 30px 0; padding: 15px 18px; '
        f'border-top: 1px solid {theme.hairline}; border-right: 0; '
        f'border-bottom: 1px solid {theme.hairline}; border-left: 3px solid {theme.subsection}; '
...
def render_opening_lead(text: str, theme: Theme) -> str:
    ...
    if theme.is_life:
        return (
            ...
            f'style="margin: 8px 0 36px; padding: 19px 9px 18px 20px; '
            f'border-top: 1px solid {theme.hairline}; border-right: 0; '
            f'border-bottom: 1px solid {theme.hairline}; border-left: 4px solid {theme.section}; '
            f'border-radius: 0; box-sizing: border-box;">\n'
            f'    <p style="margin: 0; font-size: 17px; line-height: 1.85; color: {theme.ink}; '
            f'font-weight: 700; text-align: left;">{convert_inline(text, theme)}</p>\n'
            f'</section>'
        )
    return (
        f'<section data-wa-format="opening-lead" style="margin: 12px 0 38px; padding: 17px 18px 16px; '
        f'border-top: 1px solid {theme.border}; border-right: 0; border-bottom: 1px solid {theme.border}; '
        f'border-left: 4px solid {theme.emphasis}; border-radius: 10px; box-sizing: border-box;">\n'
...
def render_image_placeholder(image_id: str, theme: Theme) -> str:
    ...
    return (
        ...
        f'  <p style="margin: 0 auto; width: 58px; padding: 7px 0; color: {theme.section}; '
        f'font-size: 15px; line-height: 1; font-weight: 700; border-top: 1px solid {theme.border}; '
        f'border-bottom: 1px solid {theme.border}; border-radius: 0;">↓</p>\n'
        f'</section>'
    )
...
def render_section_title(
    ...
    return (
        f'<section data-wa-format="section-title" style="margin: 50px 0 28px; padding: 0 0 12px 14px; '
        f'border-left: 3px solid {number_color}; border-bottom: 1px solid {theme.hairline}; '
        ...
        f'line-height: 30px; text-align: center; margin-right: 12px; vertical-align: middle; '
        f'border: 2px solid {number_color}; border-radius: 50%; background: transparent; '
...
def render_breath(text: str, theme: Theme, accent: str | None = None) -> str:
    ...
    if theme.is_life:
        return (
            ...
            f'border-left: 1px solid {accent}; border-right: 1px solid {theme.subsection}; '
...
def render_punch(text: str, theme: Theme, accent: str | None = None) -> str:
    accent = accent or theme.emphasis
    if theme.is_life:
        if nonspace_len(text) <= PUNCH_MAX_CHARS:
            return (
                ...
                f'border-left: 3px solid {accent}; border-bottom: 1px solid {theme.hairline}; '
                f'box-sizing: border-box;">\n'
                f'  <p style="margin: 0; font-size: 15px; line-height: 1.8; color: {theme.ink}; '
        ...
        return (
            ...
            f'border-top: 1px solid {accent}; border-bottom: 1px solid {theme.subsection}; '
            f'box-sizing: border-box;">\n'
    ...
    if nonspace_len(text) <= PUNCH_MAX_CHARS:
        return (
            ...
            f'style="margin: 29px 0; padding: 5px 0 7px; border-bottom: 1px solid {theme.hairline}; '
            f'box-sizing: border-box;">\n'
    ...
    return (
        ...
        f'style="margin: 33px 0; padding: 15px 18px 14px; '
        f'border-top: 1px solid {theme.hairline}; border-bottom: 1px solid {theme.hairline}; '
        f'box-sizing: border-box;">\n'
        f'  <p style="margin: 0; font-size: 15px; line-height: 1.7; '
...
def render_decorative_divider(theme: Theme, accent: str | None = None) -> str:
    """A paste-safe chapter turn: visible enough to pace, quiet enough to disappear."""
    accent = accent or theme.section
    return (
        f'<p data-wa-format="decorative-divider" style="margin: 36px 0 38px; padding: 0; '
        f'font-size: 15px; line-height: 1; text-align: center; color: {accent};">'
        f'<span style="display: inline-block; width: 42px; border-top: 1px solid {theme.hairline}; '
        f'vertical-align: middle;">&nbsp;</span>'
        f'<span style="display: inline-block; margin: 0 12px; font-size: 15px; '
        f'line-height: 1; color: {accent}; vertical-align: middle;">◇</span>'
        f'<span style="display: inline-block; width: 42px; border-top: 1px solid {theme.hairline}; '
        f'vertical-align: middle;">&nbsp;</span></p>'
    )
...
def render_chapter_lead(text: str, theme: Theme, accent: str) -> str:
    """Give every chapter a visible editorial entry without inventing copy."""
    return (
        f'<p data-wa-format="chapter-lead" style="margin: 0 0 1.55em; padding: 2px 0 2px 16px; '
        f'border-left: 3px solid {accent}; font-size: 16px; line-height: 1.9; '
        f'color: {theme.ink}; font-weight: 750; text-align: left; letter-spacing: 0;">'
        f'{convert_inline(text, theme)}</p>'
    )


def render_feature_statement(
    text: str,
    theme: Theme,
    role: str,
) -> str:
    """Render narrative milestones as distinct magazine editorial components."""
    role_styles = {
        "spark": (theme.emphasis, "life-lowpoint", "center", "18px", "850"),
        "wisdom-east": (theme.subsection, "life-wisdom-east", "left", "16px", "700"),
        "wisdom-west": (theme.section, "life-wisdom-west", "left", "16px", "700"),
        "author": (theme.emphasis, "life-author-voice", "left", "17px", "850"),
        "practice": (theme.tertiary, "life-return", "left", "16px", "800"),
    }
    accent, variant, align, size, weight = role_styles.get(
        role,
        (theme.section, "life-feature", "left", "16px", "750"),
    )
    if role == "spark":
        return (
            f'<section data-wa-format="feature-statement" data-wa-variant="{variant}" '
            f'style="margin: 40px 0; padding: 22px 18px 20px; '
            f'border-top: 3px solid {accent}; border-bottom: 1px solid {theme.hairline}; '
            f'box-sizing: border-box; text-align: {align};">\n'
            f'  <p style="margin: 0; font-size: {size}; line-height: 1.75; color: {theme.ink}; '
            f'font-weight: {weight}; text-align: {align};">{convert_inline(text, theme)}</p>\n'
            f'</section>'
        )
    if role == "author":
        return (
            f'<section data-wa-format="feature-statement" data-wa-variant="{variant}" '
            f'style="margin: 38px 0; padding: 20px 18px 19px; '
            f'border-top: 1px solid {accent}; border-bottom: 1px solid {accent}; '
            f'box-sizing: border-box;">\n'
            f'  <p style="margin: 0; font-size: {size}; line-height: 1.85; color: {theme.ink}; '
            f'font-weight: {weight}; text-align: {align};">{convert_inline(text, theme)}</p>\n'
            f'</section>'
        )
    return (
        f'<section data-wa-format="feature-statement" data-wa-variant="{variant}" '
        f'style="margin: 30px 0 34px; padding: 16px 13px 16px 19px; '
        f'border-left: 4px solid {accent}; border-top: 1px solid {theme.hairline}; '
        f'border-bottom: 1px solid {theme.hairline}; box-sizing: border-box;">\n'
        f'  <p style="margin: 0; font-size: {size}; line-height: 1.85; color: {theme.ink}; '
        f'font-weight: {weight}; text-align: {align};">{convert_inline(text, theme)}</p>\n'
        f'</section>'
    )
...
def render_blocks(
    ...
    def phase_accent() -> str:
        return {
            "descent": theme.section,
            "lowpoint": theme.emphasis,
            "rebound": theme.tertiary,
        }.get(narrative_phase, theme.section)
    ...
    for block_index, (btype, lines) in enumerate(blocks):
        if btype == "phase":
            ...
            if (
                ...
                html_blocks.append(
                    render_decorative_divider(theme, phase_accent())
                )
                divider_count += 1
            continue
        ...
        if btype == "quote":
            ...
            html_blocks.append(render_quote(text, theme, phase_accent()))
        ...
        if btype == "list":
            ...
            for item_offset, line in enumerate(lines):
                ...
                marker = (
                    ...
                    f'border: 1px solid {marker_color}; border-radius: 50%; background: transparent; '
        ...
        if btype == "table":
            ...
            if rows:
                ...
                html_blocks.append(
                    f'<section data-wa-format="table" style="margin: 18px 0 24px; overflow: hidden; '
        ...
        if pending_feature:
            html_blocks.append(render_feature_statement(text, theme, pending_feature))
        ...
        if chapter_lead_pending:
            chapter_lead_pending = False
            if not lead_used:
                lead_used = True
                html_blocks.append(render_opening_lead(text, theme))
            else:
                html_blocks.append(render_chapter_lead(text, theme, phase_accent()))
            note_body_block(text)
            continue
        ...
        can_render_breath = (
            breath_count < MAX_LIFE_BREATHS
            if theme.is_life
            else section_index not in breath_sections
        )
        if is_breath_paragraph(text) and can_render_breath:
            html_blocks.append(render_breath(text, theme, phase_accent()))
        ...
        html_blocks.append(
            f'<p data-wa-format="sentence-line" style="margin: 0 0 1.12em; '
            f'font-size: 15px; line-height: 1.9; color: {theme.ink_secondary}; '
